...
def solve(puzzle_input):
    a_room = [x for x in [puzzle_input[3][3], puzzle_input[2][3]] if x != "."]
    b_room = [x for x in [puzzle_input[3][5], puzzle_input[2][5]] if x != "."]
    c_room = [x for x in [puzzle_input[3][7], puzzle_input[2][7]] if x != "."]
    d_room = [x for x in [puzzle_input[3][9], puzzle_input[2][9]] if x != "."]
    hallway = list(puzzle_input[1][1:12])
    energy_cost = {"A": 1, "B": 10, "C": 100, "D": 1000}

    def solved(a_room, b_room, c_room, d_room):
        return a_room == ["A" for i in range(2)] and b_room == ["B" for i in range(2)] and c_room == ["C" for i in range(2)] and d_room == ["D" for i in range(2)]
        ...
        for i, amphipod in enumerate(hallway):
            ...
            if all(resting_amphipod == amphipod for resting_amphipod in target_room):
                if segment_empty(hallway_segment(hallway, i, target_pos)):
                    ...
                    hallway[i] = "."
                    return (energy_cost[amphipod] * (abs(target_pos - i) + 2-len(target_room)+1)) + make_moves(a_room[:], b_room[:], c_room[:], d_room[:], hallway[:])
            ...
            if room and room[-1] != req:
                ...
                if target_room in [[], [room[-1]]]:
                    if segment_empty(hallway_segment(hallway, pos, target_pos)):
                        ...
                        return (energy_cost[moving] * (2-len(room) + abs(target_pos - pos) + 2-len(target_room)+1)) + make_moves(a_room[:], b_room[:], c_room[:], d_room[:], hallway[:])
            ...
            if room and any(amphipod != req for amphipod in room):
                for potential in [0, 1, 3, 5, 7, 9, 10]:
                    if hallway[potential] == "." and segment_empty(hallway_segment(hallway, pos, potential)):
                        ...
                        results.append((energy_cost[moving] * (2-len(room) + abs(potential - pos))) + make_moves(a_room[:], b_room[:], c_room[:], d_room[:], hallway[:]))
                        hallway[potential] = "."
                        room.append(moving)
